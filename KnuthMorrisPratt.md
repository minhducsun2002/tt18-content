...
---
Ghi chú:
- ký hiệu `|` giữa 2 ký tự chỉ ra rằng 2 ký tự đó được biết khớp nhau thông qua so sánh
- ký hiệu `\` giữa 2 ký tự chỉ ra rằng 2 ký tự đó được biết khớp nhau __không__ thông qua so sánh
...
Thông thường, thuật toán trâu bò sẽ tiếp tục dịch đi một phần tử, so sánh
lại (và lại mismatch D:). Tuy nhiên, nếu để ý ta thấy: 
`P[0]` giống `P[1]`, do đó việc so sánh `P[0]` với `T[2]` tiếp là không tối ưu
cho lắm. Ta lại tiếp tục dịch đi (mà không so sánh) và tiếp tục khớp
`T[3]` với `P[0]`.
...
> Gọi `N(i)` là độ dài tiền tố chuẩn nhỏ nhất của `P(i)`  mà bản thân tiền tố đó cũng là
hậu tố (chuẩn) của `P(i)`.

> Bảng các giá trị `N(i)` sẽ cho ta biết sau một lần khớp, vị trí tối ưu để tiếp tục khớp là ở đâu.
